...
def build_start_keyboard() -> InlineKeyboardMarkup:
    """
    Build the main menu keyboard for the /start command.

    Includes:
    - "Open LifeQuest" button that launches the Telegram Mini App (if configured).
    - "📜 My Profile" button for character info.
    - "🗑️ Delete Account" button that triggers the deletion flow via callback.
    """
    buttons = []
...
async def start_delete_account_flow(message: types.Message, state: FSMContext):
    ...
    Clears state and prompts for the password directly.
    User is identified by their telegram_id automatically.
    """
    await state.clear()
    await message.answer(
        "⚠️ ACCOUNT DELETION\n\n"
        "You are about to permanently delete your account.\n"
        "All progress, levels, coins, and data will be LOST FOREVER.\n\n"
...
async def cmd_start(message: types.Message, state: FSMContext, session: Session):
    ...
    Clears any ongoing FSM conversation.
    - If user is registered (exists in DB by telegram_id): shows profile + main menu.
    - If user is NOT registered: starts text-based registration flow in the bot.
    """
    await state.clear()
    ...
    if not user:
        ...
        await message.answer(
            ...
            "Enter your desired login (3-20 characters, letters, numbers, -, _):"
        )
    ...
    profile_text = (
        f"⚔️ Welcome back, {user.display_name or user.username}!\n\n"
        f"📊 YOUR PROFILE:\n"
...
async def process_delete_password(message: types.Message, state: FSMContext, session: Session):
    ...
    User is looked up by telegram_id (from message.from_user.id) directly.
    Password is the confirmation of identity.
    ...
    deleted_display = user.display_name

    try:
        # ── Логіка очищення (cleanup logic) ──
        # In the future, this will also delete related records
        # (quests, achievements, inventory, etc.)
        session.delete(user)
        session.commit()
    except Exception:
    ...
    await state.clear()
    await message.answer(
        f"✅ Account '{deleted_username}' ({deleted_display}) has been permanently deleted.\n"
        f"All progress has been erased.\n\n"
        f"If you wish to return, use /start to create a new character."
    )
...
async def process_register_password(message: types.Message, state: FSMContext, session: Session):
    ...
    try:
        ...
        await message.answer(
            ...
            f"Class: {new_user.character_class} | Level: {new_user.level}\n\n"
            f"Choose your action:",
            reply_markup=build_start_keyboard(),
        )
    except Exception:
...
# ---------------------------------------------------------------------------
# Callback handlers — Main menu buttons
# ---------------------------------------------------------------------------


@router.callback_query(lambda callback: callback.data == "view_profile")
async def callback_view_profile(callback: types.CallbackQuery, session: Session):
    """
    Handle the "📜 My Profile" inline button press.

    Look up the user by telegram_id and show their character stats.
    """
    await callback.answer()
...
async def cmd_help(message: types.Message):
    ...
    await message.answer(
        "Commands:\n"
        "/start - open the game or register\n"
